//
//  EAFont.swift
//  GameSearch
//
//  Created by Ацамаз on 26.05.2025.
//

import SwiftUI


enum EAFont {
    static let blackTitle: Font = Font.custom("Helvetica-Black", size: 24)
    static let title: Font = Font.custom("Helvetica-Bold", size: 24)
    static let smallTitle: Font = Font.custom("Helvetica-Medium", size: 16)
    static let description: Font = Font.custom("Helvetica", size: 12)
    static let infoTitle: Font = Font.custom("Helvetica-Bold", size: 18)
    static let infoTitleMedium: Font = Font.custom("Helvetica", size: 18)
    static let info: Font = Font.custom("Helvetica", size: 14)
    static let infoBold: Font = Font.custom("Helvetica-Bold", size: 14)
    static let infoSmall: Font = Font.custom("Helvetica", size: 10)
    static let infoBig: Font = Font.custom("Helvetica", size: 18)

    static let navigationBarTitle: Font = Font.custom("Helvetica-Bold", size: 17)
}
