...
- [ ] **1.1** Создать `Modules/Tournaments/Shared/TierBadge.swift`.
- [ ] **1.2** Создать `Modules/Tournaments/Shared/LiveBadge.swift` (с пульсацией).
- [ ] **1.3** Создать `Modules/Tournaments/Shared/TeamLogo.swift` (с fallback gradient + initials).
- [ ] **1.4** Создать `Modules/Tournaments/Shared/DateRangeLabel.swift`.
- [ ] **1.5** Создать `Modules/Tournaments/Shared/CountryFlag.swift`.
- [ ] **1.6** Создать `Modules/Tournaments/Shared/PrizepoolLabel.swift`.
- [ ] **1.7** Создать `Modules/Tournaments/Shared/ScoreView.swift`.
- [ ] **1.8** Создать `Modules/Tournaments/TournamentsList/Views/GameSegmentControl.swift`.
- [ ] **1.9** Создать `Modules/Tournaments/TournamentsList/Views/TournamentCard.swift`.
- [ ] **1.10** Создать `Modules/Tournaments/TournamentsList/Views/LiveMatchChip.swift`.
- [ ] **1.11** Создать `Modules/Tournaments/TournamentsList/Views/LiveMatchesStrip.swift`.
- [ ] **1.12** Создать `Modules/Tournaments/TournamentsList/Views/TournamentsSkeletonList.swift`.
- [ ] **1.13** Создать `TournamentsListProtocols.swift`, `TournamentsListInteractor.swift`, `TournamentsListViewModel.swift`, `TournamentsListView.swift`.
- [ ] **1.14** Подключить пагинацию (infinity scroll).
- [ ] **1.15** Подключить pull-to-refresh.
- [ ] **1.16** Подключить empty/error states (используем модифицированный `TournamentsPlaceholderView`).
- [ ] **1.17** Заменить `ScreenFactory.makeTournamentsView()` с placeholder на `TournamentsListView`.
- [ ] **1.18** Аналитика: tournaments_tab_opened, tournaments_segment_switched, tournaments_game_switched, tournaments_pulled_to_refresh.

#### 1.B. Детали турнира

- [ ] **1.19** Создать `Modules/Tournaments/TournamentDetails/Views/TournamentHeaderView.swift`.
- [ ] **1.20** Создать `Modules/Tournaments/TournamentDetails/Views/TournamentTabPicker.swift` (использует `SwipeSegmentedView`).
- [ ] **1.21** Создать `MatchesTab.swift` (группировка по стадиям).
- [ ] **1.22** Создать `StandingsTab.swift`.
- [ ] **1.23** Создать `ParticipantsTab.swift` + `ParticipantTeamCard.swift`.
- [ ] **1.24** Создать заглушку `BracketsTab.swift` с текстом «Сетка скоро».
- [ ] **1.25** Создать `TournamentDetailsSkeleton.swift`.
- [ ] **1.26** Создать `TournamentDetailsProtocols.swift`, `TournamentDetailsInteractor.swift`, `TournamentDetailsViewModel.swift`, `TournamentDetailsView.swift`.
- [ ] **1.27** Подключить deeplink `gamesearch://tournament/<id_or_slug>`.
- [ ] **1.28** Подключить share-кнопку через `ShareLink`.
- [ ] **1.29** Аналитика: tournament_opened, tournament_tab_switched, tournament_shared.
...
_Last updated: 2026-05-25 (Phase 0 завершена)_
