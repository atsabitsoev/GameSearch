...
| `match_has_streams_count` | Записывается при загрузке деталей матча | `count: <int>`, `match_id: <int>` | ⏳ Phase 1.C |
| `match_shared` | Тап «Поделиться» матчем | `match_id: <int>` | ⏳ Phase 1.C |
...
_Last updated: 2026-05-25 (Phase 1.B v3 — добавлено событие `tournament_stage_switched` для аналитики переключений между Group Stage / Playoffs внутри серии)_
