...
| Event | Когда | Параметры | Status |
|---|---|---|---|
| `tournament_opened` | Открыт экран деталей турнира | `tournament_id: <int>`, `tournament_slug: <str>`, `from_screen: list\|live_strip\|deeplink\|favorites\|tournament` | ✅ Phase 1.A (list/live_strip) + 1.B (tournament back-nav) |
| `tournament_tab_switched` | Сменена вкладка в деталях турнира | `tournament_id: <int>`, `tab: matches\|standings\|brackets\|participants` | ✅ Phase 1.B |
| `tournament_stage_switched` | Тап на чип в `TournamentStagePicker` | `from_stage_id: <int>`, `to_stage_id: <int>`, `serie_id: <int>` | ✅ Phase 1.B v3 |
| `tournament_shared` | Тап «Поделиться» | `tournament_id: <int>`, `tournament_slug: <str>` | ✅ Phase 1.B |

### Матч

| Event | Когда | Параметры | Status |
|---|---|---|---|
| `match_opened` | Открыт экран матча | `match_id: <int>`, `status: not_started\|running\|finished\|canceled\|postponed`, `from_screen: list\|tournament\|live_strip\|deeplink\|favorites` | ✅ Phase 1.A (live_strip) + 1.B (tournament) |
| `match_has_streams_count` | Записывается один раз при успешной загрузке деталей матча (`/matches/{id}` response) | `count: <int>`, `match_id: <int>` | ✅ Phase 1.C |
| `match_shared` | Тап «Поделиться» матчем (simultaneousGesture на ShareLink) | `match_id: <int>` | ✅ Phase 1.C |

### Стрим (важный для product)

| Event | Когда | Параметры | Status |
|---|---|---|---|
| `stream_opened` | Тап на StreamRow в деталях матча — события отправляется **до** реальной попытки открыть URL (чтобы зафиксировать намерение пользователя независимо от исхода open) | `match_id: <int>`, `platform: twitch\|youtube\|other`, `language: <str>` (пусто → `"unknown"`), `is_main: true\|false`, `is_official: true\|false` | ✅ Phase 1.C |
| `stream_open_failed` | Не удалось открыть стрим (оба пути — native deeplink и fallback в Safari — упали) | `match_id: <int>`, `platform: <str>`, `reason: app_not_installed\|invalid_url\|other` | ✅ Phase 1.C |
...
_Last updated: 2026-05-25 (Phase 1.C — добавлены `match_has_streams_count`, `match_shared`, `stream_opened`, `stream_open_failed`; type-safe wrapper расширен enum'ами `StreamPlatformAnalytics`/`StreamOpenFailReason`. Также `tournaments_pulled_to_refresh` и `tournaments_error_*` теперь поддерживают `screen: match`.)_
