...
Для крупных модулей заведена отдельная база знаний в `docs/`. **Прежде чем начинать работу над модулем — прочитай его документацию**, особенно файл `00-context-for-agents.md` (точка входа в новом чате).

| Модуль | Документация | Точка входа для агента |
|---|---|---|
| Турниры (киберспорт) | `docs/tournaments/` | `docs/tournaments/00-context-for-agents.md` |

Это позволяет не терять контекст между чатами и обеспечивает архитектурную последовательность.
